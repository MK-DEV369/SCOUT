...
GKG_HEADERS = [
    ...
    "V2Persons", "Organizations", "V2Organizations", "Tone", 
...
class GDELTParser:
    ...
    def _stream_zip_csv(self, zip_path: Path) -> tuple[list[str], list[dict[str, str]]]:
        is_gkg = "gkg" in zip_path.name.lower()
        headers = GKG_HEADERS if is_gkg else EXPORT_HEADERS
        rows: list[dict[str, str]] = []
        with ZipFile(zip_path) as archive:
            csv_names = [name for name in archive.namelist() if name.lower().endswith((".csv", ".export.csv", ".gkg.csv"))]
            if not csv_names:
